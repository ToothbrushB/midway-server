...
def ping_monitoring():
    """Monitor robot connectivity in a separate thread"""
    while True:
        robots_to_remove = []
...
# Start ping monitoring in a separate daemon thread
ping_thread = threading.Thread(target=ping_monitoring, daemon=True)
ping_thread.start()